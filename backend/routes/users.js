...
// IMPORTANT: specific routes MUST come before /:telegramId routes
...
// GET /api/users/feedbacks — all feedbacks (admin) — MUST be before /:telegramId
router.get('/feedbacks', telegramAuth, requireAdmin, async (req, res) => {
  const users = await User.find({ 'feedbackHistory.0': { $exists: true } })
    .select('fullName firstName username telegramId feedbackHistory');
  const feedbacks = [];
  users.forEach(u => {
    u.feedbackHistory.forEach(f => {
      feedbacks.push({
        ...f.toObject(),
        studentName: u.fullName || u.firstName,
        username: u.username,
...
router.put('/me', telegramAuth, upload.single('profilePic'), async (req, res) => {
  ...
  const user = await User.findOneAndUpdate(
    { telegramId: req.dbUser.telegramId }, update, { new: true }
  );
  res.json(user);
});
...
// GET /api/users — list all users with search + pagination (admin)
...
// GET /api/users/:telegramId/full — full student profile for admin
router.get('/:telegramId/full', telegramAuth, requireAdmin, async (req, res) => {
  ...
  const [user, attempts, submissions, enrollments] = await Promise.all([
    ...
    QuizAttempt.find({ student: tid, status: 'submitted' })
      .populate('quiz', 'title subject').sort('-submittedAt'),
    Submission.find({ student: tid })
      .populate('homework', 'title subject dueDate').sort('-submittedAt'),
...
router.patch('/:telegramId/approve', telegramAuth, requireAdmin, async (req, res) => {
  const user = await User.findOneAndUpdate(
    { telegramId: Number(req.params.telegramId) }, { status: 'approved' }, { new: true }
  );
...
router.patch('/:telegramId/block', telegramAuth, requireAdmin, async (req, res) => {
  const user = await User.findOneAndUpdate(
    { telegramId: Number(req.params.telegramId) }, { status: 'blocked' }, { new: true }
  );
  if (!user) return res.status(404).json({ error: 'Not found' });
  res.json(user);
});
